...
| **T5** | `SOBRECARGA_POR_LLAMADA = 2_000`, no el máximo observado (1.200) redondeado a secas | Una medida anterior dio ~1.800, y la deriva entre `cl100k_base` y el vocabulario de Anthropic cambia de signo con el texto. Medido con la cuenta de trabajo: **hay que remedirlo con la cuenta de la corrida** (`verification.md` §5.2) |
| **T6** | `presupuesto` es **opcional** en `escribir_capitulo`; sin él, en serie | Hay tests que llaman al servicio sin portero; inventarles uno sería contar contra un techo que no existe |
| **T6** | El `SalidaMalFormada` del Crítico se captura también **en serie**, no solo en paralelo | Si solo se capturara en paralelo, que el capítulo cayera dependería de si el techo tenía hueco en ese instante |
| **T6** | `calidad` exporta `render_critico` y `plantilla_critico_v1` | Para contar el prompt que de verdad se envía al pedir el turno |
