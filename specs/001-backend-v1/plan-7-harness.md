...
| --- | --- | --- |
| 2026-09-24 | T3, paso 2 | **Cierra las dos desviaciones anteriores: la bandera `-deadlock` se retira.** Con las tres acciones de la regeneración la máquina queda cerrada y TLC corre **sin bandera y sin *deadlock***. Y la medición que el paso pedía (R-9): **65.601 estados distintos, 139.224 generados, profundidad 45, 3,3 s** con `-workers auto`. El paso avisaba de que `PeticionDelLector` elige entre los 31 subconjuntos no vacíos y daba salida por si tardaba más de diez minutos — **no hace falta**: tarda tres segundos, así que el modelo **conserva los 31 subconjuntos** y no se restringe a un capítulo. Cubre más de lo que la salida de emergencia habría cubierto, y no hay nada que declarar en el README por este lado. |
| 2026-09-24 | T4, pasos 4 y 6 | **Las dos ablaciones que el plan temía no dan contraejemplo, y las dos tienen un diagnóstico distinto del que el plan suponía.** Las otras dos sí caen: P3 viola `ReanudacionIntegra` y P5 viola `LecturaIgualALaVigente`.<br><br>**Paso 4 — S1.** Quitar `TodosValidados` de `Publicar` no rompe nada. El plan atribuía la causa a que «`RehacerCapitulo` no tiene rama de fallo»; la causa real es más fuerte: **en este modelo integrar y validar son el mismo evento**. `Aprobar` y `RehacerCapitulo` fijan `validado` e `integrados` de forma atómica, y `DescartarPeticion` los restaura desde una versión cuyos capítulos estaban todos validados. Luego `Pendientes = {}` **ya implica** `TodosValidados`, y la guarda es redundante. **S1 no es vacua, y está comprobado:** ablando además `Aprobar` para que integre sin validar, TLC da `Invariant PublicadaSoloConPuertas is violated`. Lo que el README debe declarar es que el modelo **no puede exhibir** un capítulo integrado sin pasar su puerta, porque fusiona `EXTRAYENDO` e `INTEGRADA` en `Aprobar` — y el sistema real sí las distingue.<br><br>**Paso 6 — *liveness*.** Reiniciar el contador en `Reanudar` no viola nada, y el plan afirma lo contrario («no rompe ningún invariante de seguridad y **sí** la *liveness*»). **Esa frase es falsa.** Quien cierra el bucle es `SF_vars(Aprobar)`: con equidad fuerte, `Aprobar` acaba ocurriendo esté como esté el contador. Comprobado bajando a `WF_vars(Aprobar)` con el contador reiniciado: `Temporal properties were violated`. Así que el contador sostiene la **cota** —`TypeOK`, cuarto ejemplo del encargo §5d— y **no** la terminación; lo que sostiene la terminación es la equidad fuerte, y eso es lo que hay que escribir en el README. Anotado por `ciberpunk-storymaker-f6` (Mario). |
| 2026-09-24 | T7, paso 8 | **P-10 no se hace, y el propio paso da derecho a no hacerlo.** La condición que ponía **sí se cumple**: ningún `agents.py` está tocado por nadie ahora mismo, comprobado con `git status`. Lo que decide es el tamaño: `SalidaMalFormada` está en **cinco** features —`calidad`, `canon`, `escena`, `obra`, `outline`— y subirla a `commons/llm/errores.py` con su reexporte por feature son diez ficheros, en cuatro features que **no son las de esta tarea**, con cinco sesiones trabajando en el árbol. El paso avisa de que «llevarse por delante el commit del hook por una limpieza sería el peor cambio posible en esta tarea», y el hook ya está hecho, verde y sin commitear. Es deuda de forma y no de comportamiento: se hace cuando el árbol esté más quieto, y sigue siendo P-10. Anotado por `ciberpunk-storymaker-f6` (Mario). |
| 2026-09-24 | T2, paso 2 | **Mismo caso que T1, y aquí el arreglo que el plan prescribe sería el incorrecto.** El paso espera «sin errores y sin `deadlock`», y dice que si aparece se arregla **en `Fin`** y nunca con la bandera. TLC da `Deadlock reached` a los 652 estados, y el estado es `estado = "PUBLICADA"` con `peticiones = 0` y `MaxPeticiones = 1`. `Fin` cubre `PUBLICADA` **solo** cuando `peticiones = MaxPeticiones`, y quien incrementa `peticiones` es `PeticionDelLector`, **de la Tarea 3**. Así que **no he tocado `Fin`**: esa guarda es deliberada —ata el final de la corrida a que las peticiones se agoten— y aflojarla hoy para silenciar un *deadlock* que causa una acción ausente metería en la especificación un permiso que la máquina terminada no debe tener, justo donde T4 comprueba la *liveness*. La instrucción del paso supone una máquina completa; con T3 sin escribir, no lo es. Se sigue con `-deadlock`: **5.823 estados distintos, profundidad 41, `TypeOK` en verde**. **La bandera se retira en T3**, y ahí sí, cualquier *deadlock* que quede se arregla en `Fin`. El control del paso 3 se hizo y funciona: con `NoPublicaNunca` añadido, TLC lo viola, o sea que **publicar es alcanzable** y los invariantes de T4 no se van a cumplir de balde. Anotado por `ciberpunk-storymaker-f6` (Mario). |
