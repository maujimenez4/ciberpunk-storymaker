...
completado: 2026-09-24    # eeb9835..f545fdb · las nueve tareas · 670 tests · CA-1 y CA-5
fecha: 2026-09-24
...
| --- | --- | --- |
| 2026-09-24 | Cierre de la ola 4 | **Dos mutaciones sobrevivieron y las dos eran huecos reales.** (1) Borrar `registrar_checkpoint` del bucle **no rompía nada**: el avance se **deriva** de `trabajo`, así que releerlo al final daba el mismo número aunque nadie lo hubiera anotado — una línea que RF-ORQ-05 exige y ningún test sostenía. T9 encadenó su valor de vuelta. (2) **`numero_de_capitulo` devolviendo el `capitulo_id` no tumbaba nada**, porque en una base recién creada los `id` son 1..10 **y los `numero` también**: `GET /trabajos/{id}` habría dicho «capítulo 47» de una novela de diez sin que fallara nada. Los tests miden ahora una **segunda** obra, donde dejan de coincidir. **Es el aviso de T7 vivo en otro sitio** |
| 2026-09-24 | Cierre de la ola 4 | **`RI-06` llevaba tres fases diciendo «legible por capítulo» sin serlo**, y ninguna tarea lo comprobó **porque ninguna tenía diez capítulos delante**. Es el patrón de esta fase dicho en una frase: *lo que no se puede ejercitar no se descubre*. Igual que la coherencia a escala, que la Fase 2 declaró explícitamente que no podía demostrar |
| 2026-09-24 | Cierre de la ola 4 | **Entra `POST /obras/{obra_id}/novela`, y la spec no lo tiene.** `CA-1` «de principio a fin» y la corrida real necesitan **una** entrada: con solo RI-05 son diez llamadas que alguien secuencia a mano sin saber los identificadores. T9 lo añadió y **pidió que lo firme alguien**, que es lo correcto. **Consecuencia para la Fase 4: la spec necesita `RI-15` y `CA-33` pasa de «los catorce endpoints» a quince.** No lo cambio ahora: modificar un requisito de una spec aprobada es lo que P-07 hizo, y aquello volvió a firma |
| 2026-09-24 | Cierre de la ola 4 | **`elementos_obligatorios` no se persiste en ninguna columna**, y la cobertura los lee del brief en bruto de `entrevista.respuestas`. Funciona de extremo a extremo por el camino de CU-01, pero **una obra creada sin entrevista no tiene elementos que cubrir — y eso es una forma de que el validador diga que sí de balde**, que es exactamente el defecto que R-6 de la Fase 1 existía para impedir por el otro lado. Es esquema sobre `features/obra`: Fase 4 |
| 2026-09-24 | Cierre de la ola 4 | **T9 me corrige el reparto, y tiene razón: «un agente por fichero dentro de la ola» se queda corto entre olas.** Tres de sus cinco encargos la obligaron a tocar ficheros de T5, T7 y T8. Iba sola, así que no hubo conflicto — pero **la regla no dice qué pasa cuando la juntura vive en fichero ajeno**, y eso ha ocurrido en las tres olas de esta fase y en dos de la anterior. Para la Fase 4: o las junturas tienen dueño explícito desde el plan, o se declara que las cierra el integrador |
| 2026-09-24 | Cierre de la ola 4 | **`CA-15` no se podía cerrar en T7, y T7 lo dijo.** Le faltaban una consulta, un punto de ejecución y un sitio donde persistir los elementos obligatorios; **dos de las tres cosas estaban fuera de su alcance de ficheros**. El criterio debería haber colgado de T9 desde el principio. Cerrado ahora, con `LEFT JOIN` **a propósito** —un hecho que ningún capítulo usa tiene que llegar a la cobertura, o la guarda que lo descarta sería inalcanzable— y devolviendo el **`numero`**, no el id |
| 2026-09-24 | Cierre de la ola 3 | **R-2 son SEIS estados no terminales, no nueve ni ocho.** El plan decía «nueve» y yo le dije «ocho» al encargar la tarea: **los dos contando estados terminales**, que no se pueden matar a mitad. De los diez de §3.3, cuatro son terminales. T8 lo contó y, mejor, **derivó `ESTADOS_VIVOS` de `ESTADOS_TERMINALES`** en vez de escribir la lista a mano, con un test que exige que el recorrido los cubra todos: **un estado vivo nuevo sin probar cae ahí y no en producción**. Ese guardia ya se ganó el sitio — con la lista vacía en el rojo, `pytest` **saltó el test parametrizado en silencio** («1 skipped») y fue el guardia quien lo delató |
| 2026-09-24 | Cierre de la ola 3 | **`architecture.md` se contradecía consigo mismo, y se corrige §3.7.** Decía «los retoma desde su último estado persistido» y §3.3 dice que un `FALLIDA` «se relanza como trabajo nuevo»: **las dos no podían ser ciertas a la vez**. Gana §3.3 porque es lo único implementable: `ejecutar_ciclo` es una sola función que recorre la máquina entera, así que volver a entrar con un trabajo que quedó en `VALIDANDO` **no lo retoma, lo empuja por transiciones que no son las suyas**. Retomar de verdad exige partir el ciclo en pasos direccionables — refactor, no detalle. La garantía que carga el peso sigue en pie: un paso interrumpido se repite entero |
| 2026-09-24 | Cierre de la ola 3 | **Cuatro mutaciones vivas en la primera pasada, y dos no eran tests que faltaban sino diseño equivocado.** M4 y M5 pedían tests. Pero M6 y M7 destaparon que `reanudar` **miraba R-6 dos veces** —por su cuenta y dentro de `empezar_capitulo`, que ya la llama— y **descartaba los trabajos vivos antes de pasar por la puerta**. Las dos cosas estaban razonadas en el docstring y **las dos razones eran falsas**. T8 rediseñó a una sola puerta, movida delante de la primera escritura: reanudar una obra detenida informa y no toca nada. Segunda pasada: las nueve tumban lo suyo y ninguna ajeno |
| 2026-09-24 | Cierre de la ola 3 | **El aviso de T6 sobre `resumen_capitulo` no se puede provocar hoy, y T8 no añadió guardia porque sería código muerto.** `consolidar_escena` y la transición a `INTEGRADA` comparten transacción, así que un capítulo no integrado **no dejó resumen**, y la reanudación arranca por el primer capítulo no integrado. En su lugar dejó el test que lo fija. **El aviso sigue vivo para la Fase 4**: regenerar un capítulo ya integrado sí llega con el resumen puesto. Es el criterio correcto — el proyecto lleva nueve restricciones inalcanzables, y añadir la décima a sabiendas habría sido peor |
| 2026-09-24 | Cierre de la ola 3 | **`Senal` no tiene causa para «el proceso murió», y se acaba usando `TIEMPO_AGOTADO` por descarte.** §3.6 no la contempla: `CONTEXTO_EXCEDIDO` sería falso y `CANCELACION` miente sobre quién lo pidió **y no tiene flecha desde `REPARANDO` ni `EXTRAYENDO`**, así que no serviría para R-2. Una `PROCESO_INTERRUMPIDO` sería más honesta; no se añadió porque `maquina.py` estaba integrado. **Es la clase de detalle que hace que una traza mienta un poco**, y con Langfuse en la Fase 4 esa traza es lo que alguien mirará |
| 2026-09-24 | Cierre de la ola 2 | **Las tres junturas que ninguna tarea podía cerrar, cerradas al integrar.** (1) `vectorizar` ya viaja del router hasta `ejecutar_ciclo`, así que **el índice se llena en producción** — T6 dejó `vectorizador_de` en `canon` a propósito, para que la traducción no se repita en cada sitio que ejecute un ciclo y acabe con un `embedding.modelo` equivocado que no rompe nada hoy y hace incomparables los vectores mañana. (2) `abrir_trabajo` rellena `trabajo.capitulo_id`, la columna que T1 añadió justo para eso y que nadie escribía: sin ella, el checkpoint de T5 no sabe de qué capítulo es cada trabajo. (3) `_nombres_del_canon` usa `leer_nombres_del_canon`, **así que `CA-16` deja de validar estricto**. Las tres cruzan features, y **es la cuarta vez en dos fases que el reparto por fichero corta justo por donde pasa el cable** |
| 2026-09-24 | Cierre de la ola 2 | **El censo dejó de decir la verdad, y no por el motivo que yo anticipé.** Avisé a T6 de que con el índice lleno el vacío dejaría de ser normal. Lo que pasó fue otra cosa: al entrar los resúmenes, **la capa pasó a tener dos almacenes**, y el cuarto caso de R-3 —hay indexadas y el almacén no devuelve nada— **ya no deja la capa vacía porque la llena el otro**. La avería habría pasado inadvertida sin que fallara ningún test. T6 bajó la comprobación a `_surtir_recuperado`. **Mi aviso apuntaba al sitio correcto por el motivo equivocado** |
| 2026-09-24 | Cierre de la ola 2 | **Séptimo y octavo test que pasaban por el motivo equivocado, y una restricción muerta retirada.** T6: un test preguntaba por `antes_de=1`, donde la función devuelve `[]` **sin llegar a la consulta**, así que la mutación del corte seguía en verde; reescrito sobre el capítulo 2. T7: `hecho.usado_en and` era **código muerto** —el bucle de dentro ya no recorre nada sobre una tupla vacía— y **lo retiró en vez de dejarlo de adorno**, comprobando con otra mutación que la propiedad sí está viva. Y T5 encontró un agujero real: sin el filtro de `obra_id` en `siguiente_capitulo` **la suite entera seguía verde**, porque ninguna otra obra tenía outline — el orquestador de una obra podía recibir el capítulo 1 de otra |
| 2026-09-24 | Cierre de la ola 2 | **T5 decidió lo de `FALLIDA` mejor de lo que la pregunta permitía.** No lo metió en `DETIENEN_LA_NOVELA` —§3.6 lo declara **relanzable**, y meterlo ahí «convertiría un remedio automático en una decisión humana»— sino que hizo que `empezar_capitulo` exija que **no quede ningún capítulo anterior sin integrar**. No mira la causa, así que frena igual un `FALLIDA` sin relanzar, un `CANCELADA` y un capítulo que nadie intentó, **y se desbloquea sola** en cuanto el relanzamiento lo integra. Y me corrige R-3: el escenario que de verdad rompe un contador global es **el capítulo sano que costó dos vueltas y se integró** |
| 2026-09-24 | Cierre de la ola 2 | **Dos junturas que pasan a T9, porque `CA-15` no está cerrado de extremo a extremo.** T7 dejó la cobertura probada y (1) **falta la consulta** que junte `hecho_canon` × `hecho_usado_en` × `capitulo.numero` para construir el `HechoUsado`, y (2) el validador **no está en el `CATALOGO`**, y «un validador que no está en el catálogo no corre»: está probado y **no lo ejecuta nadie**. Y un aviso suyo que vale para toda la fase: `usado_en` debe llevar **`numero`, no `capitulo_id`** — pasar ids no rompe ningún test y produce un informe que ninguna persona puede usar |
...
| 2026-09-24 | T8 · base | **El worktree arrancó mal, por décima vez en esta fase.** `HEAD` estaba en `7025869`, el mismo árbol ajeno que se tragó a T7: ni `src/backend/` ni `specs/`. Recolocado con `git checkout -b f3-t8-reanudacion ad9177f`. **Veinte más dos.** Ya no hay nada que aprender de la regla 5 del reparto: funciona siempre. Lo que falla es la creación del worktree, y lleva diez de diez en esta fase |
| 2026-09-24 | T8 | **La reanudación relanza el capítulo como trabajo nuevo, no retoma el trabajo muerto**, y contradice la lectura literal de `architecture.md` §3.7 («los retoma desde su último estado persistido»). El motivo es de código y no de gusto: `ciclo.ejecutar_ciclo` es **una sola función** que recorre la máquina de `PLANIFICANDO` a `INTEGRADA` llamando a `avanzar` en cada paso, así que volver a entrar con un trabajo que quedó en `VALIDANDO` no lo retoma —lo empuja por transiciones que no son las suyas hasta que la máquina se niega—. Lo que sí se cumple es la otra frase de §3.7, que es la que carga la garantía: «un paso interrumpido se **repite entero**, nunca se reanuda a medias». **Retomar de verdad desde el estado exigiría partir `ejecutar_ciclo` en pasos direccionables**, y eso es refactor de `ciclo.py`, que no es de esta tarea. Queda para quien quiera cobrar el ahorro de no repetir la llamada del Escritor |
| 2026-09-24 | T8 | **`Senal.TIEMPO_AGOTADO` se usa para cerrar el trabajo que la caída dejó vivo, y es la menos mala de las que hay.** §3.6 no tiene una causa para «el proceso murió»: `CONTEXTO_EXCEDIDO` sería un diagnóstico falso y `CANCELACION` miente sobre quién lo pidió —y además no tiene flecha desde `REPARANDO` ni desde `EXTRAYENDO`, así que no serviría para R-2—. `TIEMPO_AGOTADO` es avería, llega a `FALLIDA` desde los seis estados vivos y §3.6 le atribuye justo la propiedad que aquí importa: «si no se llegó a llamar, es relanzable sin coste». **Para quien toque `maquina.py`:** una `Senal.PROCESO_INTERRUMPIDO` sería más honesta y no se añadió porque `maquina.py` está integrado y no es de esta tarea |
| 2026-09-24 | T8 | **Los estados no terminales son SEIS, no nueve ni ocho.** El plan dice que probar R-2 en uno solo «deja nueve sin probar» y el encargo de la tarea dice «ocho»; `ESTADOS_TERMINALES` tiene cuatro de los diez de §3.3, así que los vivos son `PLANIFICANDO`, `ENSAMBLANDO`, `ESCRIBIENDO`, `VALIDANDO`, `REPARANDO` y `EXTRAYENDO`. Las dos cifras del plan cuentan estados terminales, que no se pueden matar a mitad. **R-2 se prueba en los seis**, y no con una lista escrita a mano: `ESTADOS_VIVOS` se **deriva** de `ESTADOS_TERMINALES`, y `test_r2_se_prueba_en_todos_los_estados_no_terminales` exige que los parámetros sean exactamente ese conjunto. Si mañana entra un estado vivo nuevo, el que falte se nota aquí |
| 2026-09-24 | T8 · sobre el aviso de T6 | **El `IntegrityError` de `resumen_capitulo` no se puede provocar, y se dice cómo se comprobó.** El aviso avisaba de que consolidar dos veces el mismo capítulo salta `UNIQUE(capitulo_id)`. No se ha añadido guardia, porque sería **código muerto**: `consolidar_escena` y la transición a `INTEGRADA` comparten transacción —el `commit` lo hace el `avanzar` de `_terminar`—, así que un capítulo que no llegó a integrarse **no dejó resumen**; y la reanudación arranca siempre por el primer capítulo *no integrado*, nunca por uno hecho. Lo que sí hay es el test que lo fija: `test_reanudar_no_vuelve_a_resumir_un_capitulo_ya_integrado` mata el proceso con el 1 integrado y el 2 en `EXTRAYENDO` y exige un resumen por capítulo. **El aviso sigue vivo para la Fase 4:** la regeneración de un capítulo ya integrado sí llega con el resumen puesto, y entonces la guardia habrá que escribirla — con su test, no antes |
| 2026-09-24 | T8 · sobre el aviso de T3 | **El alcance por escena de `evento` y `hecho_canon` no le hace falta a la reanudación, por el mismo motivo.** Las dos tablas solo se escriben en `EXTRAYENDO`, dentro del punto de guardado de `consolidar_escena`, así que una corrida muerta no dejó filas en ellas y no hay rastro que desambiguar por `run_id`. `descartar` **no las toca** —el ledger es *append-only*— y tampoco toca `ejecucion`, que es la traza de auditoría de la regla de dominio 7: una llamada al modelo que se pagó ocurrió aunque su corrida muriera. Lo único que se borra es `version_texto` de la corrida muerta, que es manuscrito candidato. **La deuda del aviso sigue siendo de la Fase 4**, cuando se regenere un capítulo ya integrado |
| 2026-09-24 | T8 · sobre el aviso de T5 | **`atar_al_capitulo` sigue sin tener quien la llame, y ya no hace falta.** El cierre de la ola 2 rellenó `trabajo.capitulo_id` dentro de `abrir_trabajo`, que era lo que el aviso pedía, así que la reanudación lee el capítulo de todos los trabajos sin ayuda. **`checkpoint.atar_al_capitulo` queda exportada y sin usar en producción**: o la usa quien abra trabajos por otra vía —T9, el router de regeneración— o se retira. No se retira aquí porque `checkpoint.py` está integrado y no es de esta tarea |
| 2026-09-24 | T8 | **CA-6 dejó cuatro mutaciones vivas y las cuatro eran huecos reales, no defensa redundante.** Dos eran tests que faltaban —el contador heredado por el trabajo relanzado y el regreso de la versión vigente anterior— y **dos eran diseño**: `reanudar` llamaba a `exigir_que_la_novela_siga` *y* a `empezar_capitulo`, que ya la llama, y descartaba los trabajos vivos **antes** de pasar por la puerta. Se dejó **una sola puerta**, `empezar_capitulo`, y se movió delante de la primera escritura: reanudar una obra detenida ahora informa y no toca nada. Con eso las nueve mutaciones tumban su test y ninguna tumba uno ajeno. **La técnica volvió a encontrar lo que la lectura no**: las dos de diseño estaban razonadas en el docstring y eran falsas |
| 2026-09-24 | T9 | **El bucle de los diez no es un `for`: es `reanudar` en bucle.** T8 dejó la reanudación de **un** capítulo con las cuatro cosas que el bucle necesita —elegir el pendiente, cruzar la puerta de `empezar_capitulo` (R-6, sin duplicar, sin huecos), cerrar lo que quedó vivo y abrir el trabajo con las reparaciones ya gastadas—. Un `for` de uno a diez las repetiría, y habría **dos sitios que deciden cuál es el capítulo siguiente**: el que arranca limpio y el que reanuda. Arrancar de cero y reanudar tras una caída son ahora la **misma llamada**, así que `CA-1` y `CA-5` se prueban sobre el mismo código |
| 2026-09-24 | T9 | **La condición de parada es el estado del trabajo, no un contador de vueltas.** `siguiente_capitulo` devuelve el primero **no integrado**, así que un capítulo que termina en `ESCALADA` o `FALLIDA` volvería a salir en la vuelta siguiente y el bucle no acabaría. Se para cuando el capítulo no quedó `INTEGRADA` y se devuelve en `Novela.detenida_en` con su causa (R-6: se detiene y **se informa**; no se salta al siguiente). Un tope de diez vueltas habría sido un número arbitrario disfrazado de guarda, y además habría escrito encima del hueco |
| 2026-09-24 | T9 · encargo 1 | **`CA-15` cerrado de extremo a extremo, y con una deuda descubierta al hacerlo.** La consulta que faltaba es `novela.hechos_usados`: junta `hecho_canon` × `hecho_usado_en` × `capitulo` y devuelve el **`numero`** del capítulo, nunca el `capitulo_id`. Es un `LEFT JOIN` a propósito —un hecho que ningún capítulo usa **tiene que llegar** a la cobertura, o la guarda de `cobertura.py` que lo descarta sería inalcanzable—. **Y lo descubierto:** `BriefEntrada.elementos_obligatorios` se valida al cerrar la entrevista y **no se persiste en ninguna columna**; `crear_obra_desde_brief` guarda destinatario, género, tono, calor y vetos, y los elementos se quedan por el camino. Lo único que sobrevive es el brief en bruto dentro de `entrevista.respuestas`, y de ahí los lee `elementos_obligatorios_de`. Darles columna es esquema y migración sobre `features/obra`: **queda pendiente para la Fase 4**, y hasta entonces una obra creada sin entrevista no tiene elementos que cubrir |
| 2026-09-24 | T9 · encargo 2 | **El validador de cobertura entra en un catálogo, y el catálogo es nuevo.** No cabía en `CATALOGO`: aquellos reciben un `CapituloAValidar` y devuelven `Defecto` con cita, y éste recibe el manuscrito entero y devuelve una **ausencia**, que no tiene pasaje que citar. Se añade `PuntoDeEjecucion.PUERTA_G4` —el punto que `verification.md` §8.1 ya le declaraba— y `CATALOGO_DE_MANUSCRITO`, con el nombre que ese documento le da: **`cobertura_de_personalizacion`**, no el de la función. Lo ejecuta `cerrar_manuscrito`, y `novela.cobertura_de_la_novela` pasa por ahí y no por la función suelta: llamarla directamente la dejaría corriendo **fuera** del catálogo, que es el otro lado de la misma frase. Toca `calidad/{validadores,cobertura,__init__}.py`, fuera del alcance nombrado de la tarea, y va dicho: nadie más corría en la ola 4 |
| 2026-09-24 | T9 · encargo 3 | **`checkpoint.atar_al_capitulo` se retira, con su test.** El aviso de T8 admitía dos desenlaces y se buscó el otro primero: el bucle de la novela abre sus trabajos con `abrir_trabajo`, como todos, así que **no hay quien abra trabajos por otra vía**. Una función pública sin ningún llamador es una invitación a abrir un trabajo por una segunda puerta, que es justo lo que no debe existir. El test se va con ella y no para que pase el código: lo que probaba ya no existe. El porqué queda escrito en el docstring de `checkpoint.py` (`CLAUDE.md` §3.3) |
| 2026-09-24 | T9 | **Un endpoint nuevo, `POST /obras/{obra_id}/novela`, y no está en la tabla de RI de la spec.** El plan nombra «su router» en el alcance de la tarea, y `CA-1` —«de principio a fin»— y la corrida real declarada necesitan **una** entrada: con solo RI-05 la novela son diez llamadas que alguien tiene que secuenciar a mano, y quien la lance no sabe los identificadores. Arranca **y reanuda** por la misma ruta, porque son la misma llamada. Va en el `router.py` que ya existe y no en uno nuevo: la feature tiene un solo router y `main.py` ya lo monta, así que **`main.py` no se toca** —el alcance lo nombraba y no hizo falta—. **Queda para la spec**, que es quien decide sus interfaces |
| 2026-09-24 | T9 | **RI-06 decía «legible por capítulo» y no lo era.** `EstadoDelTrabajo` devolvía `escena_id` y nada más: con diez trabajos abiertos no había forma de saber cuál era el del capítulo siete. Ahora lleva `capitulo_id` **y** `numero_de_capitulo`, y el segundo no es adorno: es la misma advertencia que T7 dejó escrita para `usado_en` —un identificador no le dice nada a quien lee—. Y la advertencia estaba viva: la mutación que devuelve el `capitulo_id` en lugar del número **no tumbaba ningún test**, porque en una base recién creada los diez capítulos tienen `id` 1..10 y `numero` 1..10. El test se rehízo sobre una **segunda** obra, donde los dos dejan de coincidir |
| 2026-09-24 | T9 · CA-6 | **Once mutaciones, dos supervivientes en la primera pasada y las dos eran huecos reales.** (1) Borrar la llamada a `registrar_checkpoint` del bucle no tumbaba nada: el avance se **deriva** de `trabajo`, así que releerlo al final daba el mismo número aunque nadie hubiera anotado. Quedaba una línea que RF-ORQ-05 exige y que ningún test sostenía; se encadenó su valor de vuelta y ahora sin la llamada no hay checkpoint que devolver. (2) La del `numero_de_capitulo`, arriba. Y **una mutación que mata por el motivo ajeno, dicha en vez de descubierta**: quitar la parada del bucle tumba el test de R-6, pero lo que salta es el `CheckpointPrematuro` de `checkpoint.py`. La parada sigue siendo suya —sin ella no hay `Novela.detenida_en` que informar—, y la mitad «no se escribe el siguiente» la sostienen **dos paredes**, como ya pasaba en `ciclo.py` |
| 2026-09-24 | T9 · corrida real | **La corrida real declarada NO se ejecuta**, igual que en T1 y T11 de la Fase 2: gasta cuota de la cuenta y es decisión del dueño, no de un agente. El comando queda en el informe, sin correr |
