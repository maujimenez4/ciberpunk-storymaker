...
| 2026-09-22 | P-122 | **`verificar.sh` no funcionaba en un clon limpio**, por dos motivos que solo se ven clonando. Invocaba `python -m uv`, que solo existe si `uv` se instalo con pip dentro del interprete: el instalador oficial y Homebrew dejan un binario, asi que en una maquina recien preparada fallaba con «No module named uv» antes de comprobar nada. Y estaba commiteado como `100644`, sin bit de ejecucion, asi que `./verificar.sh` daba «Permission denied». Se comprobo clonando de verdad desde el remoto y ejecutandolo ahi. |
