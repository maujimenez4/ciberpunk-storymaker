...
| --- | --- | --- |
| 2026-09-24 | Cierre de la fase | **`Entrevista` entra en `docs/definitions.md` (v2.3) y la tabla deja de ser un término sin definir.** T9 la implementó porque sin ella no había ni un test, y dejó la definición **propuesta** sin tocar `docs/` — hizo lo correcto: `CLAUDE.md` §2 dice que un término ausente no se introduce sin que una persona lo confirme, y ningún agente aprueba. Confirmada por `maujimenez4` el 2026-09-24. Entra en §9.1, junto a `TextoAportado` y `Comprador`, con `entrevista_id`, `respuestas`, `obra_id` (0..1) y `cerrada_en` (0..1) |
| 2026-09-24 | Cierre de la fase | **`HechoCanon` toma la forma que declara `definitions.md` §4.5: `entidad`, `atributo`, `valor`, `confianza`, en lugar de un `enunciado` de texto libre.** No es cosmética, y por eso se hace aquí y no se deja para la Fase 2: la **regla de arbitraje** del propio §4.5 —«si dos hechos sobre el mismo **atributo** difieren, prevalece el de menor `orden_discurso`»— y el defecto `CAN-01` comparan atributos, y sobre una cadena libre no hay nada que comparar. Se aplazó al cierre a propósito, porque T7, T8 y T9 escribían contra `enunciado` en olas vivas y renombrar debajo de ellos es la rotura que el reparto evita. Con ello **R-2 cambia de sitio y mejora**: el hecho en blanco ya no lo filtra el repositorio mirando `enunciado.strip()`, lo para `HechoDelBrief` **antes de construirse**, porque sus tres campos son `TextoNoVacio`. Y `recuerdos` pasa a `recuerdos_aportados`, como §9.1 |
| 2026-09-24 | Cierre de la fase | **Los tests no habrían cazado el renombrado a medias; las puertas sí.** Al cambiar la firma faltaron los `import` de `HechoDelBrief` en `repository.py` y `service.py`, y **la suite siguió en verde**: Python 3.14 evalúa las anotaciones de forma diferida (PEP 649), así que un nombre sin importar en una firma no revienta en ejecución. Lo cazaron `ruff` (F821) y `mypy` (`name-defined`). Es el argumento de la Tarea 1 —las puertas antes que el dominio— cobrado con intereses al final de la fase |
...
| 2026-09-24 | T10 · Interfaces | **Dos de los tres nombres que el plan declara no están en `docs/definitions.md`, y esta vez sí se renombran.** El plan pide `FilaAuditoria` y un quinto parámetro `detalle`; §9.3 del documento llama a la clase `RegistroDeAuditoria` y al campo `evidencia`. `CLAUDE.md` §2 es inequívoco —gana el documento— y además prohíbe introducir términos que no estén en él. **Se renombra ahora, al revés que en T4, y la diferencia es de reparto y no de criterio:** los nombres de T4 los consumían T7, T8 y T9 en olas vivas, y cambiarlos era la rotura que el reparto existe para impedir; a `registrar` **no lo llama nadie todavía** —RF-GUA-04, que es quien lo llamará, es de la Fase 2—, así que el coste de renombrar hoy es cero y mañana no. La aridad, el orden y la posición de los parámetros no cambian, de modo que los tres tests del plan siguen valiendo letra por letra. La clase se llama como las cinco de T4, que también se llaman como su entrada del documento |
| 2026-09-24 | T10 · Interfaces | **`sujeto` se queda fuera, y hay que decidirlo en la Fase 2 a sabiendas.** `definitions.md` §9.3 da al `RegistroDeAuditoria` seis atributos —`registro_id`, `momento`, `sujeto`, `decision`, `motivo`, `evidencia`— y la firma del plan no tiene por dónde decir **sobre qué** se decidió: `obra_id` dice de quién es el registro, no qué unidad se juzgó. Hoy no se nota porque en esta fase la única unidad que existe es la obra, así que `obra_id` **hace de `sujeto` y el registro sigue siendo legible**. Deja de serlo en cuanto haya capítulos: diez decisiones sobre la misma obra y ninguna que diga sobre cuál. No se añade una columna que nada puede rellenar —sería peor que no tenerla—, y queda escrito para que quien traiga el ciclo de capítulo lo encuentre planteado y no de golpe |
| 2026-09-24 | T10 · Migración | **La primera de las dos trampas de Alembic que T4 anotó está a medias, y conviene afinarla porque induce a error.** La fila de la ola 3 dice que `--autogenerate` **no compara `CheckConstraint`**, y es cierto exactamente para una tabla que **ya existe**: una restricción añadida después no sale sola, que es lo que le pasó a `hecho_canon`. Pero una tabla **nueva** se emite con su `__table_args__` entero, así que el `ck_registro_auditoria_decision` de esta migración salió generado y no a mano. Se comprobó **leyendo el esquema de la base migrada** con `sqlite3`, no fiándose de la salida del generador, y se dejó escrito en la propia migración para que el siguiente no lo quite creyendo que sobra. La segunda trampa sí mordía: la clave ajena salió **sin nombre** y se le puso uno (`fk_registro_auditoria_obra_id`) al crearla, para que la primera migración que recree esta tabla en modo batch no tenga que rescatarla con un `naming_convention` como hubo que hacer con `hecho_canon` |
| 2026-09-24 | T10 · Paso 1 | **CA-6 no tenía sobre qué aplicarse: la restricción que el plan exige es la única regla de esta tarea sin test.** El apartado **Interfaces** dice «`decision` en `{permitido, bloqueado}`», y los tres tests del plan escriben solo valores válidos: quitar el `CheckConstraint` del modelo y de la migración dejaba la suite **entera en verde**. Se añade `test_una_decision_que_no_es_ni_permitido_ni_bloqueado_no_entra`, contra la base y no contra un validador de Python, porque `registrar` no será la única puerta el día que un servicio inserte por su cuenta. Con él, CA-6 muerde: quitada la restricción, **1 failed, 3 passed**, y el que cae es ese. Los tres tests del plan quedan intactos |
| 2026-09-24 | T10 · Paso 1 | **El tercer test del plan nace en verde, y no es un test de esta tarea.** `test_ninguna_clave_se_lee_del_repositorio_ni_de_la_base` (RF-OBS-07) asevera sobre el `Ajustes` de T1 y pasó **en la primera ejecución, antes de que existiera una sola línea de `auditoria.py`**. No es un defecto —es un guardaespaldas de regresión legítimo, y RF-OBS-07 está en la lista de la tarea—, pero conviene que esté dicho: el rojo→verde de T10 lo demuestran los otros tests, no ese, y un recuento que lo incluya como prueba del ciclo estaría contando de más. Es el mismo género de aviso que el plan ya se hizo a sí mismo en T4 · Paso 2 |
| 2026-09-24 | T10 · Pasos 2-4 | **El registro es *append-only* por convenio de función, no por esquema, y eso es menos de lo que RF-GUA-05 dice.** El plan cierra el invariante con `borrar_auditoria`, que existe solo para lanzar `OperacionNoPermitida`; y funciona para quien la llame. Pero la tabla acepta `DELETE` y `UPDATE` de cualquier otra ruta —un `sesion.delete(fila)`, un `UPDATE` a mano—, así que lo que hay es una puerta con cartel, no una pared: justo la distinción que `CLAUDE.md` §11 hace al exigir que los guardarraíles vivan «en código, antes de aceptar», y no en la buena voluntad de quien pasa. Dos disparadores de SQLite (`BEFORE DELETE`/`BEFORE UPDATE` con `RAISE(ABORT)`) lo cerrarían en el esquema y costarían poco. **No se hace aquí**: el alcance declarado de la tarea son tres funciones, el plan no da test para ello y una restricción sin test no entra por TDD. Queda para quien cierre la fase, con la pregunta de fondo: si el registro es la prueba de por qué salió la novela, su integridad no debería depender de qué función se use para tocarlo |
| 2026-09-24 | T10 · Pasos 2-4 | **Dos huecos menores que aparecieron al desplegar la línea «falla, implementar, verde, migración».** (1) El plan no dice si `registrar` confirma o solo vuelca: hace `flush` y **no** `commit`, porque `commons/db/sesion.py` deja esa decisión al servicio, y sin el `flush` la fila no existiría para el `select` de `leer_auditoria` dentro de la misma transacción. (2) `registrar` **no tiene por dónde recibir el reloj**, que `CLAUDE.md` §6 manda inyectar: la firma del plan tiene cinco parámetros y ninguno es el `Reloj` de T3, así que `momento` sale de `RelojDelSistema()` por dentro. Añadir un sexto habría cambiado una firma pública que el plan fija y ningún test cubriría. Es la misma deuda que T4 dejó con `datetime.now(UTC).date()` dentro del `model_validator`, y se salda igual: cuando esto lo llame un servicio, el reloj entra por ahí. `leer_auditoria` ordena por `id` y no por `momento`, que es lo que hace que el orden no dependa del reloj |
| 2026-09-24 | T9 · Paso 3 | **CU-01 no se puede ejecutar fuera de la suite: esta fase nunca construye un `ClienteModelo`.** El plan cierra RI-01, RI-02, RI-03 y «CU-01 entero», y los tres endpoints necesitan al Entrevistador, que necesita un cliente de modelo. La única implementación de `ClienteModelo` que existe al terminar la fase es `DobleDeterminista`, y ninguna tarea tenía asignado el proveedor real: ni la 3 —«Reloj, cliente y doble», que entrega el **protocolo** y el doble— ni ninguna otra. Se añade `obtener_cliente_modelo()` en `commons/llm/cliente.py` como la dependencia por la que entrará (RI-14), y **levanta `NotImplementedError`**: en pruebas se sustituye por `dependency_overrides`. Comprobado contra la aplicación levantada de verdad: `POST /entrevistas` responde 201, y `/respuestas` y `/cerrar` responden **500** con ese `NotImplementedError` resuelto por FastAPI *antes* de entrar en la ruta. RI-13 sí se cumple: el OpenAPI publica los tres caminos con sus modelos. Se descartó traducirlo a un 501 amable: un hueco de fase se cierra en la fase siguiente, y una respuesta ordenada lo haría más fácil de no ver. **Es lo primero que tiene que resolver la Fase 2**, y «Lo que esta fase NO hace» no lo menciona |
| 2026-09-24 | T9 · Paso 3 | **`Entrevista` no es una clase de `docs/definitions.md` §9**, y `CLAUDE.md` §2 dice que un término que no está no se introduce sin proponer la definición. La tabla es obligatoria —el plan se la da a T9 y sin ella no hay ni un test— y «entrevista» ya está en la spec como proceso (RI-01 a RI-03, RF-ENT-01 a RF-ENT-08), así que se implementa. Queda **la propuesta, sin escribir en `docs/`**, porque un agente no la aprueba: `Entrevista` · *Conversación con el comprador mientras aún no hay `Obra`. Acumula sus respuestas y el `TextoAportado`; al cerrarse produce la `Obra` y guarda su id, que es lo que hace que cerrar dos veces no cree dos* · `entrevista_id`, `respuestas`, `obra_id` |
| 2026-09-24 | T9 · Paso 3 | **El plan no dice de dónde sale `obra.titulo`, y la columna no admite nulo.** El brief no trae título y el Arquitecto —quien lo pone— es de la Fase 2. Se deriva del destinatario (`Novela para <nombre>`) como título provisional. No es prosa generada (RD-06): es el nombre con el que la obra se encuentra hasta que tenga el suyo. Si la Fase 2 prefiere `titulo` nulo hasta el Arquitecto, es una migración de una columna |
| 2026-09-24 | T9 · Paso 1 | **La fixture `cliente` que da el plan no basta: sobrescribe la sesión y no el cliente de modelo.** Con solo esa sobrescritura, los cuatro tests del plan llaman al proveedor real y la suite deja de cumplir CA-4 y RNF-FIA-01 —sin red y sin credenciales—; hoy, además, levantan `NotImplementedError`. La fixture añade `dependency_overrides[obtener_cliente_modelo]` con un `DobleDeterminista`, y las respuestas preparadas se declaran en una fixture `respuestas_del_modelo` que cada módulo de tests sobrescribe: vacía por defecto, para que un test que llame al modelo sin prepararlo falle en vez de recibir algo inventado |
| 2026-09-24 | T9 · Paso 1 | **Cinco tests más de los cuatro que pide el plan, y cuatro de ellos porque CA-6 no tenía sobre qué caer.** (1) `cerrar` con un faltante que el esquema **no** ve —`recuerdos` tiene valor por defecto en `BriefEntrada`—: sin él, quitar la guarda de faltantes no hacía caer ningún test, porque la validación de esquema tapaba el agujero y el 409 salía igual. (2) y (3) `TextoAportado` se guarda (RF-ENT-05) y un texto en blanco no lo crea (R-2). (4) el `CheckConstraint` de `texto_aportado` probado **por inserción directa**: con solo el test anterior, quitar la restricción del esquema no hacía caer nada, que es justo lo que CA-6 existe para detectar. (5) los vetos del brief llegan al ámbito `brief` (RF-ENT-02). Y un sexto: una entrevista inexistente da 404 y no 500 |
| 2026-09-24 | T9 · Paso 1 | **Un test del plan pasaba en verde con los tres endpoints sin escribir.** El de la entrevista inexistente miraba solo el código de estado, y sin ruta registrada FastAPI ya devuelve 404 por su cuenta: en el rojo salió `8 failed, 1 passed`. Se le añadió la comprobación del cuerpo —que el `detail` nombre el id— y entonces el rojo fue de 9. Es el mismo fallo que CA-6 busca, por la puerta del transporte en vez de la del dominio |
| 2026-09-24 | T9 · Paso 3 | **`EntrevistaDesconocida` no estaba en el plan**, que solo autorizaba `BriefIncompleto` y `BriefContradictorio`. Sin ella, un id inventado revienta con un `AttributeError` sobre `None` y el comprador recibe un 500 por haber escrito mal una URL. Cuelga de `ErrorDeDominio` como las otras dos y es la única que el manejador central no traduce a 409: lo que falla no es el estado, es el recurso |
| 2026-09-24 | T9 · Paso 3 | **El `ValidationError` del esquema trae faltantes y contradicciones mezclados y el plan no dice cómo se cuentan.** Se parte por el tipo del error: `missing` es RF-ENT-03 y va a `BriefIncompleto`; todo lo demás —la regla de dominio 6, la edad contra la fecha de nacimiento (R-7), el veto que choca con el nombre (R-8)— es RF-ENT-04 y va a `BriefContradictorio`. Meterlo todo en un `detail` dejaría al comprador sin saber si tiene que aportar un dato o corregir otro, que es lo que CA-2 pide poder distinguir. Y en `cerrar` las contradicciones se miran **antes** que los faltantes: una contradicción no se arregla aportando el dato que falta |
| 2026-09-24 | T9 · CA-6 | **Quitar la guarda del texto en blanco hace caer seis tests, no uno.** No es un test mal escrito: sin ella, cada petición sin texto intenta insertar un `TextoAportado` vacío, el `CheckConstraint` lo rechaza y el 500 arrastra a todo lo que pase por `/respuestas`. Las otras cuatro mutaciones —guarda de R-5, guarda de contradicciones, guarda de faltantes, escritura de los vetos— hacen caer **exactamente su test y ninguno más**, y quitar el `CheckConstraint` hace caer solo el de inserción directa. Queda anotado porque la regla de CA-6 es «su test y solo el suyo», y esta cumple lo primero y no lo segundo |
| 2026-09-24 | T9 · Paso 5 | **La comprobación del OpenAPI con la aplicación levantada estuvo a punto de dar un falso negativo.** El puerto 8123 que manda el encargo de la tarea seguía ocupado por un `uvicorn` de una ola anterior, sirviendo la aplicación **sin rutas**: `curl` respondió 200 con `"paths": {}` y el `uvicorn` nuevo murió con «address already in use» en su propio log, que nadie mira. Se repitió en otro puerto y salieron los tres caminos. Para quien cierre una fase: comprobar que el proceso que responde es el que se acaba de arrancar |
