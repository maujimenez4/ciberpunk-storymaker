...
**Se espera al menos una, y conviene decirlo:** el cliente de T2 se genera de un OpenAPI que **hoy no existe**. Cuando la Fase 3 del backend lo produzca, los nombres de campo de este plan podrán ser otros. El paso 5 de T2 —`pnpm gen:api` y `git diff --exit-code`— es exactamente el que lo destapará.
