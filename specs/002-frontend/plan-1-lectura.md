...
| 2026-09-24 | T1 · cuarta zona | **`except` no admite comodín, y con el del plan la regla cierra la puerta que `RF-FRO-03` deja abierta.** El plan escribía `except: ["*/index.ts"]`. Medido con los dos casos sobre `eslint-plugin-import` 2.32: con el comodín, el import interno **y** la entrada legítima por `index.ts` salen los dos bloqueados; con la lista explícita —`["manuscrito/index.ts", "canon/index.ts"]`— solo el interno. Es el mismo fallo que el plan ya había corregido una vez en esta zona, un nivel más abajo: la redacción anterior dejaba pasar lo que debía prohibir, y esta prohibía lo que debía dejar pasar. **Lo destapó el cuarto test**, el control negativo que este plan no tenía: con los tres del plan, la zona quedaba «verde» bloqueándolo todo. Coste de la lista: una feature nueva no entra sin añadirse aquí, y falla **cerrada**. |
| 2026-09-24 | T1 · el test | **`import/no-restricted-paths` solo dispara si el import resuelve, y el fichero del plan no existía.** El tercer test importa `features/manuscrito/components/Portada`, que es de T5: como el módulo no estaba, el resolver no resolvía, la regla callaba y el test leía ese silencio como frontera cerrada. Es un falso verde que dice justo lo contrario de lo que pasa. `lintDeUnFichero` acepta ahora los **módulos destino** y los crea junto al fichero de prueba. |

| 2026-09-24 | T4 · `Muestra.tsx` | **Añadida una pantalla de muestra que no está en el plan, y es andamio.** `src/app/Muestra.tsx` aplica los primitivos y los tokens sobre contenido de una novela cualquiera —dedicatoria, tres párrafos con sangría, índice, los dos avisos, botón y enlace— para que la dirección estética **se pueda mirar** antes de que existan T5, T6 y T7, que leen datos que el backend aún no publica. Pedida por el usuario a través del orquestador: quería ver el avance hoy. **No es una página del producto y se retira cuando T3 monte el router.** Mientras exista, `main.tsx` la monta en la raíz. |
| 2026-09-24 | T4 · el foco | **`getComputedStyle(nodo, ":focus-visible")` no comprueba nada bajo jsdom.** El test del plan lee así el contorno del foco, y jsdom devuelve vacío para cualquier pseudo-clase: la aserción `outlineStyle !== "none"` **pasa con el foco quitado**. Se comprueba sobre la hoja de estilos, que es donde la regla vive; el foco sobre un navegador real es de T8, con `axe`. |
| 2026-09-24 | T4 · paso 5 | **`axe` bajo jsdom tampoco comprueba el contraste**, que es justo lo que el paso 5 pide medir con herramienta y no a ojo: la regla `color-contrast` necesita un motor que pinte, así que en la suite **no corre** y `toHaveNoViolations` pasa sin haber mirado el color. Entra `contraste.test.ts`, que calcula el ratio de la WCAG sobre los tokens leídos del CSS y comprueba los **seis** pares de la paleta: prosa, metadatos y enlaces sobre el papel, texto del botón sobre el acento, y prosa y metadatos sobre la guarda. Los seis pasan AA. No sustituye a T8: adelanta la comprobación al sitio donde se elige el color. |

| 2026-09-24 | T3 · novedades | **Resuelta la desviación que quedaba abierta, y la respuesta es que no hace falta ningún andamio.** `/l/{t}/novedades` tiene un **estado vacío propio y permanente**: `RF-IND-03` dice que en la primera versión publicada no hay marca de cambio porque no hay anterior con la que comparar, así que **toda novela recién regalada abre aquí y lee exactamente esto**, también cuando la Fase 3 esté hecha. Lo que la Fase 3 añade es el otro caso, no este. Dice: «Esta es la primera versión de tu novela, así que no hay nada que comparar todavía. Si pides un cambio, aquí verás qué capítulos se reescribieron» — un estado vacío es una invitación a actuar, no un hueco. Con eso `CA-21` deja de cumplirse por vacío en su quinta ruta. |
| 2026-09-24 | T3 · la muestra | **`Muestra.tsx` sale de la raíz y queda en `/muestra`, solo en desarrollo.** El plan prometía retirarla cuando T3 montara el router. Borrarla habría tirado lo que el usuario acaba de aprobar, y dejarla accesible la habría convertido en «una página del producto que nadie especificó». Va tras `import.meta.env.DEV`, así que **el `build` la deja fuera**: comprobado sobre `dist/` —cero coincidencias de su contenido—. No puede quedarse por descuido, porque en producción no existe. Se borra del todo cuando T5, T6 y T7 pinten las páginas reales. |
| 2026-09-24 | T3 · ficheros | **Entra `src/app/Aplicacion.tsx`, que el plan no listaba.** El plan daba `router.tsx` y `providers.tsx`; falta el árbol que los junta y registra las rutas. Se separa de `main.tsx` a propósito: `main.tsx` monta sobre el DOM y no se puede renderizar en un test, y el test de T3 necesita montar la aplicación entera para comprobar que las cinco rutas **responden** y no solo que las cadenas se construyen bien. |
| 2026-09-24 | T1 · el test de fronteras | **Sube el límite a 30 s.** Cada caso arranca ESLint entero sobre el proyecto, y al crecer las dependencias pasó de los 5.000 ms por defecto de vitest. La lentitud es inherente a lo que ese test hace —ejecutar la herramienta en vez de leer su configuración—, así que bajar el límite habría significado dejar de ejecutarla. |
