...
| Integración en CI/CD | Hacer pasar los cambios generados por el agente por la misma tubería que los escritos por personas | **Previsto (agente de código)** — sin una vía aparte y más débil para los diffs del agente. La tubería es la lista de `CLAUDE.md` §15: `ruff`, `mypy`, `pytest`, `lint-imports`, `pnpm typecheck`, `pnpm lint` y migraciones de Alembic. Exigencia propia de este proyecto: la tubería incluye además `corrida.py --seco`, la demostración de CA-1 con dobles. *La exigencia anterior —correr en los dos modos de `VectorStore`— se retiró con D-02 el 2026-09-22: no hay extensión vectorial que degradar* | [Continuous integration](https://en.wikipedia.org/wiki/Continuous_integration) |
...
| El sistema funciona **sin ninguna credencial en el proceso** | arq. §2 | **T + D** | La suite corre sin red y con dobles; además, el proceso arranca sin una sola variable de credencial. El proveedor es el CLI de Claude Code y autentica con la sesión de la cuenta, fuera del proceso. *Decía «con una sola credencial» hasta la decisión (a) del 22-09-2026, que borró las tres que ningún código leía* |
...
| 3.2 | La verificación formal y la comprobación de modelos siguen descartadas, pero la decisión deja de ser genérica: §2 y §3 enlazan el catálogo de quince lenguajes con veredicto por fila, y la comprobación de modelos nombra su herramienta para el día que se reabra. De ese catálogo salió la spec 002, con seis defectos de los validadores mecánicos reproducidos |
