...
- **Qué falta:** los datos obligatorios del brief que no están, y **solo esos**. Del
  destinatario, `nombre` y `edad`; de la obra, `genero`, `tono`, `nivel_de_calor` y
  `elementos_obligatorios`.

  **No pidas nada que no esté en esa lista.** Ni extensión, ni número de capítulos, ni
  presupuesto: lo que no está ahí, o lo decide el sistema o es opcional. Un dato de más
  en `faltantes` **detiene la creación de la obra**, y el comprador se queda mirando un
  formulario que le pide algo que nadie va a usar.

  Son opcionales y **no se piden nunca**: `rasgos`, `recuerdos_aportados`,
  `fecha_de_nacimiento`, `vetos`, `dedicatoria` y el texto aportado.
