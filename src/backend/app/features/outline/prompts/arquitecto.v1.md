...
- `capitulos`: lista de exactamente diez objetos. Cada uno con estas claves y ninguna más:
  ...
  - `titulo`: cadena.
  - `pov_dominante`: el nombre de **un** personaje. Uno solo.
  - `lugar`: dónde ocurre.
  - `objetivo`: qué persigue el POV en este capítulo.
  - `obstaculo`: qué se lo impide.
  - `valor_entrada` y `valor_salida`: el giro de valor previsto. **Distintos entre sí.**
  - `gancho_de_apertura`: con qué abre.
  - `tipo_de_corte_final`: cómo corta — en tensión, en pregunta o en revelación.
